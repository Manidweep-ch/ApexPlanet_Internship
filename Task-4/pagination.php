...
// we set the number of results to display per page for pagination purposes. We then execute a SQL query to fetch all posts from the database and display them in a structured format. Each post is displayed with its title, content, creation date, and action buttons for editing and deleting the post.
        ...
        if(isset($_GET['type']) && $_GET['type'] === 'my')
        {
            if(!empty($where))
            {
                $query1 = "SELECT COUNT(id) AS total FROM posts WHERE (title LIKE :search_term OR content LIKE :search_term) AND user_id = :user_id";
            }
            else
            {
                $query1 = "SELECT COUNT(id) AS total FROM posts WHERE user_id = :user_id";
            }
        }
        ...
        if(isset($_GET['type']) && $_GET['type'] === 'my')
        {
            $stmt1->bindValue(':user_id',$_SESSION['user_id'],PDO::PARAM_INT);
        }
        if(!empty($where))
            {
                $search_term="%$search%";
        ...
        $stmt1->execute();
        //
        $total_results = (int) $stmt1->fetch(PDO::FETCH_ASSOC)["total"];
        ...
        if(isset($_GET['type']) && $_GET['type'] === 'my')
        {
            if(!empty($where))
            {
                $query2 = "SELECT * FROM posts WHERE (title LIKE :search_term OR content LIKE :search_term) AND user_id = :user_id ORDER BY created_at DESC LIMIT :start,:result_per_page;";
            }
            else
            {
                $query2 = "SELECT * FROM posts WHERE user_id = :user_id ORDER BY created_at DESC LIMIT :start,:result_per_page;";
            }
        }
